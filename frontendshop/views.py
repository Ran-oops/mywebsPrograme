...
def detail(request, tid):
    goods = GoodsInfo.objects.filter(id=tid)
    return render(request, 'frontend/detail.html', {'goods': goods})
    # return HttpResponse('hi, detail')
...
def shopcartadd(request, tid):
    return HttpResponse('hi, add to cart')
